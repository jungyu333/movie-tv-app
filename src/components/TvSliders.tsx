...
import { IgetTvOnTheAirResults } from "../api";
...
function TvSliders({ data, title, sliderNum, ClickSliderNum }: slidersProps) {
  const navigation = useNavigate();
  const [index, setIndex] = useState(0);
  const [leaving, setLeaving] = useState(false);
  const [back, setBack] = useState(false);
  const detailMovieMatch = useMatch("/tv/:tvId");
  const increaseIndex = () => {
    if (data) {
      if (leaving) return;
      toggleLeaving();
  ...
  const onClickkBox = (tvId: number) => {
    navigation(`/tv/${tvId}`);
  };
  const onClickExit = () => navigation("/tv");
  const clickedMovie =
          ...
          >
            ...
            >
              {data?.results
                ...
                .map((tv) => (
                  <Box
                    layoutId={tv.id + "" + String(sliderNum)}
                    onClick={() => onClickkBox(tv.id)}
        ...
        {detailMovieMatch && ClickSliderNum === Number(sliderNum) ? (
          <>
            <Overlay animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <BigMovieContainer
                style={{ top: "8vh" }}
                layoutId={detailMovieMatch.params.tvId + String(sliderNum)}
              >
                {clickedMovie && (
                  <>
                    <BigCover>
